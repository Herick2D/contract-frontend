...
export function Sidebar() {
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-50 w-72 bg-white border-r border-gray-100 flex flex-col">
      {/* Logo */}
        ...
        {navigation.map((item) => {
          ...
          return (
            ...
            >
              ...
              <item.icon className={cn('w-5 h-5 relative z-10', isActive && 'text-gray-700')} />
              <span className="relative z-10">{item.name}</span>
              {isActive && (
                <motion.div
                  layoutId="activeIndicator"
                  className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-gray-700 rounded-full"
                  transition={{ type: 'spring', stiffness: 400, damping: 30 }}
                />
              )}
            </Link>
